...
async function generateCodeWithBedrock(prompt: string): Promise<string> {
  console.log("🤖 Generating code with AWS Bedrock Claude (streaming)...");
  ...
  const input = {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 60000,
      temperature: 1,
      system: systemPrompt, // Make sure this is defined in your scope
      messages: [
        {
          role: "user",
  ...
  try {
    // Use the streaming command instead
    ...
    for await (const chunk of response.body) {
      if (chunk.chunk?.bytes) {
        ...
        if (
          ...
          if (chunkCount % 10 === 0) {
            // console.log(
            //   `[Streaming] Received ${chunkCount} chunks, current length: ${fullResponse.length} chars`
            // );
          }
        }
      }
    ...
    console.log(
      `✅ Code generation completed with ${fullResponse.length} total characters in ${chunkCount} chunks`
    );
    return fullResponse;
  } catch (error) {
...
async function processPromptMessage(message: PromptMessage): Promise<void> {
  const buildId = message.job_id;
  const sessionId = sessionManager.generateSessionId();
  const startTime = Date.now();

  console.log(
    `[${buildId}] 🚀 Starting processing for prompt: "${message.prompt.substring(
      0,
      100
  ...
  try {
    ...
    try {
      ...
      console.error(
        `[${buildId}] ❌ Failed to create temp directory:`,
        dirError
      );
      throw new Error(
        `Failed to create temp build directory: ${
    ...
    console.log(
      `[${buildId}] 🤖 Generating frontend code with AWS Bedrock Claude...`
    );
    ...
    try {
      ...
      console.log(
        `[${buildId}] ✅ Code generation completed with ${claudeResponse.length.toLocaleString()} characters`
      );
    } catch (genError) {
      console.error(`[${buildId}] ❌ Code generation failed:`, genError);
      throw new Error(
        `Failed to generate code with Bedrock: ${
    ...
    try {
      ...
      try {
        ...
        console.log(
          `[${buildId}] 🔍 Saved raw response for debugging at: ${debugPath}`
        );
      } catch (debugError) {
    ...
    if (failedFiles.length > 0) {
      console.error(
        `[${buildId}] ⚠️ Failed to write ${failedFiles.length} files`
      );
    } else {
      console.log(
        `[${buildId}] ✅ Successfully wrote ${writeResults.length} files`
      );
    }
    ...
    try {
      distUrl = await Promise.race([
        ...
        new Promise<never>((_, reject) => {
          setTimeout(
            () =>
              reject(
                new Error("Azure Container Job timed out after 5 minutes")
              ),
            5 * 60 * 1000
          );
        }),
      ...
      console.error(
        `[${buildId}] ❌ Azure Container Job failed:`,
        containerError
      );
      throw new Error(
        `Failed to execute Azure Container Job: ${
    ...
    try {
      ...
      console.error(
        `[${buildId}] ❌ Failed to parse container job response:`,
        parseError
      );
      throw new Error(
        `Invalid response from container job: ${
          parseError instanceof Error ? parseError.message : "Unknown error"
        }`
      );
    }
    ...
    console.log(
      `[${buildId}] ✅ Build process completed successfully in ${totalTime}s!`
    );
    ...
    const totalTime = ((Date.now() - startTime) / 1000).toFixed(2);
    console.error(
      `[${buildId}] ❌ Build process failed after ${totalTime}s:`,
      error
    );
...
async function main() {
  ...
  const receiver = serviceBusClient.createReceiver("prompt_queue", {
  ...
  try {
    ...
    const messageHandler = async (message: any) => {
      ...
      try {
        ...
        const messageBody: PromptMessage = JSON.parse(data);

        console.log(
          `📝 Processing prompt: "${messageBody.prompt.substring(
            0,
            100
          )}..." "project job.id ${messageBody.job_id}"`
        );
